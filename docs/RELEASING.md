...
Before the readiness run, verify and record each control in issue `#133` with
the verifier, date, and a settings-page link or redacted screenshot:
...
1. Complete every issue and pull request in the release milestone without
   bypassing checks, except for exactly one open, non-pull-request issue: issue
   `#133`, labeled `release-tracker`. Any other open issue or pull request,
   another issue with the label, an unlabeled `#133`, or multiple tracker
   exceptions blocks readiness and release. Both workflows paginate the full
   milestone result before applying this rule. Close an item only after its
   implementation, documentation, regression tests, dead-code review, and
...
5. Confirm the `0.2.0 CLI` milestone has no open issues or pull requests except
   the single permitted `#133` `release-tracker` issue.
...
The workflow rechecks milestone closure with only the exact `#133`
`release-tracker` exception and refuses every other open item. Local worktrees
