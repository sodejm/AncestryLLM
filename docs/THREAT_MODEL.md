...
| Control | Source and runtime evidence | Packaged evidence and residual ownership |
|---|---|---|
| `TM-O01`, `TM-O02` | Electron Main creates one random UUIDv4 per launch and sends it only through the exact private sidecar launch frame. Strict TypeScript and Python validation rejects missing, malformed, and additional fields. Main, packaged-sidecar, and Python-core writers accept only the versioned schema and stable code catalog, cap serialized records at 4 KiB, write separate JSONL streams, rotate three 512 KiB files, and refuse symlinked directories and active files. Persisted-file tests scan for credential, path, genealogy, prompt, response, port, URL, stderr, and output canaries. The UUID is correlation-only and is not a user, device, installation, authorization, or domain identity. | Exact-head packaged evidence remains a PR and release gate. Diagnostics have no telemetry, network, export, upload, CI-artifact, release-artifact, or automatic support-bundle path. |
