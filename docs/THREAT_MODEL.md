...
|---|---|---|
| `TM-F02`, `TM-L01`, `TM-O01`, `TM-U01`, `TM-U03` | One orchestrator captures the closed manifest into isolated staging, validates exact regular-file inventory and PNG signatures, scans raw assets for privacy canaries, requires every Markdown reference and meaningful alt text to match manifest ownership, and publishes only the complete allowlisted set transactionally. Check mode copies only tracked working-tree source into temporary state, recaptures all scenarios, compares exact bytes, and leaves the checkout unchanged. Hosted failure evidence follows a closed schema and contains only scenario IDs, surface identifiers, per-scenario status, overall status, and expected and observed SHA-256 values; pixels, transcripts, response bodies, fixtures, environment, usernames, hostnames, and absolute paths are excluded. CI fixes locale, timezone, viewport, Node, pnpm, Playwright/Electron lock, virtual display, fonts, and digest-pinned terminal dependencies, and retains the hash-only report only on failure. | Publication contains only reviewed fictional `provider=none`, network-disabled states. The Electron adapter denies renderer requests and the terminal container remains network-disabled; neither receives credentials, real genealogy data, host home state, or provider configuration. The orchestrator is repository documentation tooling and changes no application API, command registry, provider contract, GEDCOM representation, RootsMagic mutation, storage schema, FastAPI route, Electron bridge, or release artifact. A missing dependency, changed pixel, unverified platform, incomplete capture, report-schema error, or privacy-canary match fails closed. |
