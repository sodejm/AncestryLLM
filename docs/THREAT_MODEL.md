...
| `TM-S01` | The packaged sidecar selects keyring-only secret resolution, so environment injection remains available only to CLI/headless/CI consumers and cannot satisfy a packaged desktop credential. Normal launches inherit only the minimal platform environment. The exact internal Linux verification marker permits Electron main to forward only the disposable verifier's D-Bus address and home/XDG directories; the marker itself, provider credentials, `PATH`, and alternate Python keyring selectors remain excluded. Missing, locked, denied, or unavailable keyring state fails closed and never causes credential or database-key creation. The renderer retains only presence and startup status. | The Ubuntu exact-head packaged and installer checks use a disposable, owner-only native GNOME Secret Service on a private D-Bus session; they prohibit alternate Python keyring backends and retain no keyring state. Source tests prove that the child-process allowlist opens only for the exact marker and rejects credential and backend-selector canaries. Broader native keyring denial/locking cases and canary scans across packaged UI, process environment, crash/support artifacts, and release evidence remain #123/#131. |
