...
| `TM-R02`, `TM-L02` | A closed design-system boundary rejects Node/Electron imports, direct bridge use, renderer network primitives, raw HTML sinks, and production fixture imports. The production build verifier rejects remote endpoints, remote assets, fictional fixture copy, and the development gallery. Stable coded-error views render reviewed React nodes only; the implementation ships no model-output surface. | Packaged adversarial model Markdown and AST allowlisting remain #112/#131. |
...
| `TM-S01` | The packaged sidecar selects keyring-only secret resolution, so environment injection remains available only to CLI/headless/CI consumers and cannot satisfy a packaged desktop credential. Normal launches inherit only the minimal platform environment; Linux additionally retains the active D-Bus session address and XDG runtime directory required to reach native Secret Service. The exact internal Linux verification marker additionally permits only the disposable verifier's home and XDG cache, configuration, and data directories. The marker itself, provider credentials, `PATH`, and alternate Python keyring selectors remain excluded. Missing, locked, denied, or unavailable keyring state fails closed and never causes credential or database-key creation. The renderer retains only presence and startup status. | The Ubuntu exact-head packaged and installer checks use a disposable, owner-only native GNOME Secret Service on a private D-Bus session; they prohibit alternate Python keyring backends and retain no keyring state. Source tests distinguish the production session locators from the exact-marker-only verifier directories and reject credential and backend-selector canaries. Broader native keyring denial/locking cases and canary scans across packaged UI, process environment, crash/support artifacts, and release evidence remain #123/#131. |
...
| `TM-V01`, `TM-F02` | Writable startup inventories SQLCipher user tables once, creates revision `0002` only when the workspace is truly empty, and performs no DDL for a complete current schema. Exact revision `0001` is the only automatic migration. Unknown revisions plus unversioned, missing, or unexpected tables fail with `DATABASE_MIGRATION_REQUIRED` before implicit repair. Regression tests prohibit per-table reflection and prove interrupted layouts remain unchanged. | A public migration/recovery command, interruption evidence against production-sized data, and packaged backup/restore remain #123/#131. |
...
- The current shutdown path drains the Uvicorn listener/server, stdio, complete
  ...
  pre-spawn work and waits for any launch already in flight before declaring the
  process tree empty. The initial quit is vetoed while cleanup runs; only the
...
| `AB-19` | A registry, mutable tag, wrong-architecture image, bootstrap package, or package repository supplies a compromised deployment. | Medium / Critical | `TM-B01`, `TM-U01`, `TM-U02`, `TM-U03`; #306, #132, #353, #358-#362; G7. Negative: digest/platform/provenance/SBOM/license mismatch, rollback/freeze, mirror substitution, unsafe archive/hook, and interrupted acquisition preserve the trusted version. | Partially evidenced: #306 covers the verified `uv` trust chain, while #349 builds and tests exact native amd64/arm64 image digests and records complete Python and Debian package/license inventories. The rating is not reduced because OCI base and published-image provenance, SBOM publication, signing, registry acquisition, rollback/freeze protection, and complete exact-head release evidence remain #132/#353/#358-#362 and G7. |
