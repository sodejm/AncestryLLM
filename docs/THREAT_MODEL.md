...
routes. The source-level Issue #352 implementation extends that grant broker
with one transport-neutral mediated-operation request/result contract and a
Main-owned allowlisted broker. Local work receives only private `0700` staging
with immutable `0400` inputs and exact mounts verified against realized runtime
state; remote work receives one-use bounded streams and opaque metadata, never
local paths. It bounds counts, bytes, time, and concurrency, validates every
output before atomic publication, and cleans exact private state on success,
cancellation, failure, expiry, and startup. It adds no renderer route, domain
API, concrete genealogy adapter, or supported container workload. The `0.6.0`
source-level Issue #105 implementation adds
...
Issue #352 file data crosses from native grants into private Main-owned staging,
...
| A09:2025 Security Logging and Alerting Failures | Stable error codes and privacy-minimal structural evidence; payload/access logging is off; bounded per-component desktop files use a non-identity launch UUID, refuse symlinks, and have no export or upload path; container, reverse-proxy, installer, and support output forbid secrets, paths, genealogy data, prompts, responses, ports, and bearers (`TM-O01`, `TM-O02`, `TM-V01`, `TM-G01`). | Canary-secret scans across UI, API, bounded desktop files, container/proxy stderr, Docker metadata, package logs, crash/support bundles, build artifacts, and release evidence; retention, writer-failure, and fixed bridge-action tests. |
...
flowchart LR
    ...
    Main["Electron main BFF\nfile-grant and mediated-operation brokers"]
    ...
    Staging["Private operation staging\n0700 roots; immutable inputs"]
    ...
    Files -->|"opaque native grant"| Main
    Main -->|"source-level private staging"| Staging
    Staging -->|"exact local mounts or one-use remote streams"| Workers
    Workers -->|"declared output staging"| Main
    Services --> Workers
    Services <--> Storage
    Services <--> Providers
    API <--> PluginHost
...
flowchart LR
    ...
    HostFiles["Native-dialog granted host files\nuntrusted"]
    Staging["Private per-operation staging\n0700 roots; immutable inputs"]
    ...
    Broker --> Worker
    Worker <--> Volume
    HostFiles -->|"opaque grant"| Main
    Main -->|"stage and revalidate"| Staging
    Staging -->|"exact input RO and output RW mounts"| Worker
...
| `TM-F01` | Opaque file grants and operations: native dialogs create high-entropy, window/operation-scoped, expiring, revocable grant IDs; shared DTOs and renderer surfaces receive only opaque operation/artifact references, never unrestricted paths. A remote adapter receives only bounded streams and opaque metadata. |
| `TM-F02` | Backend file safety: regular-file, link, realpath/fingerprint, replacement, case-folding, Unicode-collision, and archive-signature checks; ingress budgets; source/output non-aliasing; immutable private staging; exact local mounts; validation of every declared output before atomic per-destination publication; and failure cleanup. |
...
| `TM-D01` | Availability: bounded request/event/file sizes, file counts and aggregate bytes, archive expansion and depth, queues, workers, operation time and concurrency, replay, memory/cost/token limits, cancellation, and deterministic overload errors. Public file boundaries use the typed limits and race checks in [bounded file ingress](reference/FILE_INGRESS.md). |
...
| `TM-U06` | Release-quality evidence governance: one schema-v1 policy assigns QA, security, performance, and diagnostics owners and binds exact commands, tool versions, readiness and desktop receipt gates, coverage policy, target-specific performance ceilings, diagnostic schema and retention, and bounded exceptions. A closed verifier requires the two declared artifacts, their canonical policy identity, schema version, and digest, and their exact release commit; release re-downloads the immutable inputs, compares the assembled readiness evidence to its approved copy, regenerates the approval, and verifies every assembled asset's build provenance before publication. |
...
| `TM-K01` | Runtime and container isolation: native architecture, rootless or VM-isolated engine where supported, non-root users, read-only roots, dropped capabilities, `no-new-privileges`, default-deny seccomp/MAC policy, no host network/PID/IPC/devices/privilege, explicit resource limits, bounded logs, and platform-native firewall/socket tests. The source-level mediated mount policy permits only exact private per-operation input-read-only and output-read-write bindings resolved by Main and compares the complete realized mount set; broad, additional, missing, writable-input, read-only-output, aliased, or mismatched mounts fail closed. The static #349 Compose topology continues to permit no host path. Rootful-daemon, Docker-group, WSL2, Hyper-V, VM, named-pipe, user-namespace, and kernel limitations are disclosed and tested rather than assumed equivalent. |
...
| `TM-O01` | Privacy-minimal observability: allowlisted stable codes and hashes/counts only by default; no secrets, unrestricted paths, genealogy values, prompts, responses, or bootstrap material. Issue #461 adds source-tested, bounded per-component desktop JSONL streams with one non-identity launch UUID, symlink refusal, and fixed no-argument open/clear actions. | Target-matched packaged and release-artifact evidence remains tracked in Issue #131 residual controls; local diagnostic files are never collected as CI or release artifacts. |
...
### Issue #461 structured desktop diagnostic evidence

| Control | Source and runtime evidence | Packaged evidence and residual ownership |
|---|---|---|
| `TM-O01`, `TM-O02` | Electron Main creates one random UUIDv4 per launch, derives one absolute diagnostics directory beneath its application-data root, and sends both only through the exact private sidecar launch frame. Strict TypeScript and Python validation rejects missing, relative, malformed, and additional fields; the Python writers use that exact Main-derived directory. Main, packaged-sidecar, and Python-core writers accept only the versioned schema and runtime code allowlist, cap serialized records at 4 KiB, write separate JSONL streams, rotate three 512 KiB files, and refuse symlinked directories and active files. Persisted-file tests scan for credential, path, genealogy, prompt, response, port, URL, stderr, and output canaries. The UUID is correlation-only and is not a user, device, installation, authorization, or domain identity; the directory remains private bootstrap control data and never crosses the renderer bridge. | Exact-head packaged evidence remains a PR and release gate. Diagnostics have no telemetry, network, export, upload, CI-artifact, release-artifact, or automatic support-bundle path. |
| `TM-I01`, `TM-F02` | Two fixed, zero-argument bridge actions let authorized renderer Main ask the OS to open the app-owned diagnostic directory or clear the bounded files. Main derives the directory; the renderer cannot provide or receive a path, URI, filename, record, or general filesystem capability. Existing sender, frame, origin, route, structured-clone, and operation limits remain authoritative, and directory/file symlinks fail closed. | The operating-system account boundary still permits the local user or an administrator to inspect files. Users must treat manually shared files as confidential; no application export flow weakens that responsibility. |
| `TM-D01`, `TM-C01` | Schema, destination, rotation, serialization, and write failures degrade to stable writer-unavailable/writer-degraded events when possible and otherwise fail silent without blocking verification, spawn, readiness, retry, shutdown, termination, or authorized exit. The exact stderr shutdown receipt remains a separate Main-owned check; diagnostic JSON and arbitrary child output cannot satisfy it or influence lifecycle authority. Tests cover writer failures across Electron and Python plus lifecycle, restart, exhaustion, manual retry, session invalidation, bridge rejection, and shutdown. | Disk exhaustion or an unavailable destination may remove local diagnostic evidence but cannot relax security decisions or prevent safe termination. Lifecycle state and exact shutdown-receipt validation remain authoritative. |

Impact assessment: Issue #461 adds bounded local storage and two fixed native
bridge actions, but no service, domain, public API, renderer filesystem,
provider, or network authority. The review considered privacy leakage,
correlation reuse as identity, launch-frame replay, symlink redirection, disk
growth, arbitrary filesystem access, writer-induced availability failure,
diagnostic data mistaken for shutdown authority, and artifact or upload
collection. Strict schemas, per-launch rotation, fixed Main-owned paths,
symlink refusal, bounded retention, non-blocking writers, separate shutdown
receipt validation, and the absence of export, upload, telemetry, and CI
collection address those paths. Residual exposure is limited to processes that
already share the local operating-system account or have administrator access.

### Issue #462 release-quality evidence

| Control | Source and workflow evidence | Security and architecture disposition |
|---|---|---|
| `TM-U06` | The closed schema-v1 policy enumerates the four quality families, their owners and commands, pinned toolchain, complete readiness and desktop receipt inventories, and the only accepted input and output artifact paths. Readiness schema v2 and desktop aggregate schema v3 each bind the canonical policy identity, schema version, and digest. The verifier rejects unknown fields, malformed or missing gates, failed results, wrong commits, mismatched policy references, substituted tools or targets, and incomplete, expired, or unapproved exceptions. Release consumes the exact-head artifacts, emits one deterministic approval, then re-downloads those immutable inputs, checks the assembled readiness evidence against its approved copy, regenerates the approval, and verifies build provenance for every assembled release asset before publication. | [Issue #462](https://github.com/sodejm/AncestryLLM/issues/462) changes repository delivery governance only. GitHub Actions, hosted runners, authorized maintainers, and the integrity and availability of retained artifacts remain trusted; missing or unverifiable state blocks release. |
| `TM-O01`, `TM-O02` | The required diagnostics receipt uses deterministic fictional canaries against the fixed schema digest and safe-code inventory. The policy records three 512 KiB files per component as the local retention ceiling and requires telemetry to remain disabled. Release evidence contains only the bounded receipt result, not diagnostic records, paths, genealogy values, prompts, responses, credentials, or user identity. | This adds no diagnostic export, upload, support-bundle, network, or telemetry path. The local operating-system account and administrators retain access to local diagnostic files as described by Issue #461. |
| `TM-D01`, `TM-U04` | Native packaged rows measure cold launch, warm launch, readiness, resident memory, and renderer outbound requests through one declared unpacked-native method and target-specific ceilings. Desktop and release jobs retain their separately governed literal timeouts. | These ceilings are hosted release evidence, not a runtime scheduler, availability guarantee, or permission to weaken job timeouts. Runner variance or an unavailable target leaves evidence incomplete rather than authorizing a release. |

Impact assessment: Issue #462 adds policy-bound release evidence and no
application API, CLI, DTO, provider, GEDCOM, storage, FastAPI, Electron, or
runtime authority. The review considered stale and wrong-head evidence,
artifact substitution or editing, omitted gates, toolchain drift, target or
budget drift, privacy canaries entering artifacts, provenance bypass, and
exception bypass. Closed schemas, canonical policy references, artifact
digests, exact-head binding, a second pre-publish evidence verification,
per-asset build-provenance verification, fictional canaries, and fail-closed
exceptions address those paths. A passing approval supports a delivery
decision but does not prove the absence of defects or vulnerabilities.

### Issue #200 durable mutation evidence

The application mutation port and private account-scoped journal extend `TM-C01`
and `TM-F02` to settings replacement, shared artifact/RootsMagic export
publication, and sync update/rebase. Deterministically ordered OS locks remain
held after lease expiry; resource revisions, owner tokens, fences, and newly
authorized recovery bindings prevent a second cooperative native process from
publishing under stale ownership. The SQLite journal stores only coordination
...
### Issue #352 mediated host-file evidence

| Control | Source and runtime evidence | Packaged evidence and residual ownership |
|---|---|---|
| `TM-I01`, `TM-F01`, `TM-O01` | One strict transport-neutral request/result DTO carries only an operation ID, transport choice, opaque grant references, artifact references, status, and path-free progress/error metadata. The Main-owned broker has a closed operation allowlist. Local adapters receive private staging details that never cross the shared DTO; remote adapters receive only one-use bounded streams and opaque metadata. Tests reject malformed transport contracts, replays, stale grants, and path-bearing public results. | Issue #114 supplies the concrete source-level GEDCOM adapter without a renderer route. Target-matched parser-worker and packaged operation evidence remain #118/#131. |
| `TM-F02`, `TM-C01`, `TM-K01` | Main creates exact `0700` per-operation roots with immutable `0400` inputs, rejects symlinks, hard links, special files, replacement races, case-folding and Unicode aliases, and archive signatures, and validates every declared output before atomic per-destination publication. Local plans admit only exact input-read-only and output-read-write binds and compare the complete realized mount set; cancellation, failure, expiry, and startup cleanup remove only exact private operation state and preserve unexpected entries fail-closed. The existing allowlisted read-only `family_trees` mount remains the static deployment input policy; mediated transient mounts do not widen it. | Issue #114 supplies single-artifact GEDCOM publication and cancellation preservation. A concrete parser worker, integrated container runtime, whole multi-output transaction semantics across destinations, and packaged recovery evidence remain #118/#131/#364/#365. |
| `TM-D01` | The operation allowlist fixes input/output counts and kinds; the broker enforces aggregate input bytes, zero archive expansion/depth, two concurrent operations, a five-minute deadline, bounded one-use streams, single-use operation IDs, and stable cancellation/overload outcomes. Boundary, one-over, replay, timeout, cancellation, and concurrency tests exercise the source policy. | Calibrated native/package limits and broader parser/runtime adversarial evidence remain #131/#364/#365. |

### Issue #114 transport-neutral GEDCOM operation evidence

| Control | Source and runtime evidence | Packaged evidence and residual ownership |
|---|---|---|
| `TM-F01`, `TM-F02`, `TM-O01` | Typed inspect, merge, subtree, quality, and sync requests accept only purpose-bound opaque grants and return bounded path-free DTOs, progress, and stable coded errors. Registry resolution rejects symbolic links, hard-link aliases, canonical-identity changes, replacements, and oversize inputs. Output is staged, validated, cancellation-checked, and atomically published while preserving an existing destination on failure or cancellation. | Source-level application and authenticated API evidence is complete for this boundary. Parser-worker isolation and target-matched packaged mediation remain #118/#131. |
| `TM-L01`, `TM-L02` | Optional adjudication requires an explicit provider and model through the modular `LLMService` plus existing policy and consent checks. `provider=none` is deterministic and socket-denial tests prove it remains offline with ambient keys. A model cannot make an undeclared destructive choice; `MergeDecisionRequest` declares `retain-both` as the conservative missing/cancelled-decision default. | Native target-matched provider-network instrumentation remains #131. |
| `TM-D01` | The shared job manager bounds workers and pending work, locks conflicting grant resources, forwards cooperative cancellation, exposes stable overload and domain codes, and withholds typed results until completion. Cancellation is represented as cancellation rather than generic failure. | Parser-process isolation and calibrated packaged limits remain #118/#131/#364/#365. |
...
### Issue #465 screenshot-quality and scope evidence

| Control | Source and test evidence | Security and architecture disposition |
|---|---|---|
| `TM-F02`, `TM-L01`, `TM-O01` | The canonical inventory removes terminal media in favor of selectable instructions. Closed inclusion and appearance records require reviewed exceptions; viewport-bounded crops and actual PNG bytes enforce static images, width, size, and 144-dpi metadata before publication and drift comparison. Fresh Electron captures require Ubuntu 24.04 x86_64 before staging or installation and pin project appearance while varying light and dark host preferences. Existing canary, network-denial, fictional-fixture, path-containment, and atomic-publication controls remain in force. | This is repository tooling only. It changes no application runtime, UI theme system, command registry, provider behavior, genealogy or credential data flow, storage, API, Electron bridge, release artifact, or provenance boundary. Terminal adapter security tests retain a separate non-publication fixture manifest. Image quality does not establish target-matched packaged product support. |
...
- Before forced termination, Electron Main invalidates the public active session
  and may use only its captured credentials for one exact authenticated,
  bodyless, hidden runtime-shutdown request. The renderer and preload receive no
  route or credential authority. Uvicorn's graceful-exit signal starts the
  normal lifespan drain, but the HTTP response is not proof: Main independently
  observes the leader exit and falls back to the existing full-tree termination
  path when the request fails or the process remains live.
- The current shutdown path drains the Uvicorn listener/server, stdio, complete
  ...
  can yield. Stop cancels pre-spawn work and waits no more than 20 seconds for
  ...
  send `SIGTERM` through the shared production signal-to-quit path. That process
  must report the exact native result `{ code: 0, signal: null }`.
...
| `STR-F-S` | Spoofing: a symlink, replacement race, device, alias, archive, or stale grant makes a different file appear selected. | `TM-F01`, `TM-F02` | #103, #352, #114, #131 / G1 | Negative: link/device, case-folding/Unicode/realpath alias, archive signature, replace-after-grant, expired/cross-window grant, and fingerprint mismatch are rejected. |
| `STR-F-T` | Tampering: a source or existing output is overwritten, partially published, or changed during processing. | `TM-F02`, `TM-C01` | #103, #352, #114, #118 / G1 | Negative: source/output alias, concurrent writer, source-hash change, immutable-stage mutation, worker death, disk-full, and publish failure preserve sentinels. |
...
| `STR-F-I` | Information disclosure: records, notes, keys, paths, plaintext databases, scratch files, streams, or backups escape. | `TM-S01`, `TM-F01`, `TM-F02`, `TM-O01`, `TM-O02` | #103, #352, #123, #131, #366 / G1 | Negative: shared DTOs, remote adapters, progress, errors, backup collisions, and job snapshots remain path-free; canaries are absent from logs and temp remnants; plaintext, weak keyring fallback, broad permissions, and unencrypted backups fail. |
| `STR-F-D` | Denial of service: huge, recursive, corrupt, compressed, locked, replayed, or device-like input consumes memory, CPU, disk, or workers. | `TM-D01`, `TM-F02` | #352, #114, #118, #131 / G2 | Negative: boundary/one-over size/count/depth/ratio/time/concurrency, parser complexity, replay, cancellation, queue-full, and scratch-quota tests remain bounded. |
| `STR-F-E` | Elevation: parser features, SQLite extensions, path traversal, broad mounts, or inherited worker capabilities reach the host. | `TM-F02`, `TM-D01` | #103, #352, #114, #118 / G1 | Negative: traversal, extra or mismatched mounts, extension loading, shell metacharacters, unexpected environment/socket/process access, and malformed native input fail closed. |
...
| `AB-04` | A malicious or replaced GEDCOM exploits parser complexity, symlinks, aliasing, races, or partial publication. | High / High | `TM-F01`, `TM-F02`, `TM-D01`, `TM-C01`; #103, #114, #118, #131, #352; G1/G2. Negative: boundary/one-over, replacement races, worker failure, output locks, cancellation, and sentinel preservation. | Partially evidenced: #103 proves path-free native selection, purpose and format checks, single-use lifecycle revocation, fingerprint and replacement-race rejection, source/output alias denial, output locking, cancellation, and sentinel preservation at source level. Its dedicated cross-platform packaged scenario proves native open/save mediation, path-free DTOs, explicit replacement confirmation, and revocation. #352 adds private staging, link/replacement/case-folding/Unicode/archive rejection, explicit count/aggregate-byte/archive/time/concurrency bounds, exact mount-plan/runtime comparison, immutable RootsMagic staging, validate-all output handling, atomic per-destination publication, path-free progress/errors, and cleanup at source level. #114 adds source-level typed GEDCOM operation services and a bounded job façade with purpose grants, path-free DTOs, stable codes, offline `none`, conservative retain-both decisions, cancellation preservation, and single-artifact atomic publication. Concrete parser-worker isolation, integrated runtime/package evidence, and transaction-wide recovery across multiple destinations remain #118/#131/#364/#365. |
...
| `AB-13` | A daemon, VM, container runtime, or kernel compromise escapes isolation and reaches host or genealogy data. | Medium / Critical | `TM-H01`, `TM-K01`; #348, #349, #352, #364, #365; G5/G7. Negative: privileged/root execution, host namespaces, devices, broad mounts/capabilities, socket access, and unsupported runtime modes are rejected. | Partially evidenced: #349 proves non-root, read-only, capability-free, no-new-privileges images with no host namespace, device, host path, or Docker-socket mount on native Linux amd64 and arm64. #352's source policy generates only exact private input-read-only and output-read-write mounts and rejects an incomplete, additional, broad, aliased, or mismatched realized set before publication. It does not integrate with or prove a container engine. The rating is not reduced because daemon, VM, kernel, seccomp/MAC, hostile workload, and integrated host-runtime evidence remain pending under #364/#365 and G5/G7. |
...
| `AB-23` | Release-quality evidence is incomplete, stale, edited, substituted, bound to another commit or policy, or bypassed with an invalid exception. | High / High | `TM-U06`; #462 and #131; G2/G7. Negative: closed-schema fixtures cover unknown and missing fields, gate and tool drift, failed receipts, wrong commits and digests, target and ceiling drift, and missing, expired, or unapproved exceptions; release re-downloads both immutable inputs, compares the assembled readiness evidence to its approved copy, and regenerates the approval immediately before publication. | Reduced to Medium only while the exact-head policy-bound approval passes both verifier invocations. Authorized maintainer or hosted-runner compromise and GitHub artifact integrity remain trusted residuals; unavailable or unverifiable evidence blocks release. |
...
| Gate | Exit evidence |
|---|---|
| `G0 — Architecture` | #98 and #346 record assets, actors, flows, STRIDE threats, AB-01 through AB-23, controls, risk owners, OWASP/NIST mappings, rejected designs, negative tests, issue/path ownership, quantitative budgets, and reviewer decision. |
