...
|---|---|---|
| `TM-U06` | The closed schema-v1 policy enumerates the four quality families, their owners and commands, pinned toolchain, complete readiness and desktop receipt inventories, and the only accepted input and output artifact paths. The verifier rejects unknown fields, malformed or missing gates, failed results, wrong commits, mismatched policy digests, substituted tools or targets, and incomplete, expired, or unapproved exceptions. Release consumes the exact-head readiness and desktop aggregates, emits one deterministic approval, and verifies all three retained files again immediately before publication. | [Issue #462](https://github.com/sodejm/AncestryLLM/issues/462) changes repository delivery governance only. GitHub Actions, hosted runners, authorized maintainers, and the integrity and availability of retained artifacts remain trusted; missing or unverifiable state blocks release. |
