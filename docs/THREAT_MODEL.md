...
| A03:2025 Software Supply Chain Failures | Locked dependencies, minimal extras, pinned CI actions, audits, SBOM/provenance, digest-pinned OCI/runtime/bootstrap/package inputs, signed production packages, sidecar manifests, and rollback protection (`TM-U01`, `TM-U02`, `TM-P02`, `TM-B01`). | Lockfile and image-layer review, dependency audits, CodeQL, Semgrep, secret scan, CycloneDX SBOM, digest/architecture/provenance, signature, rollback, and revoked-artifact tests. |
...
| A08:2025 Software or Data Integrity Failures | Source hashes, SQLCipher integrity, validated DTOs/model output, atomic writes, sequenced events, digest-pinned Compose/OCI/runtime/package inputs, signed production packages/updates, and anti-rollback state (`TM-F02`, `TM-E01`, `TM-U01`, `TM-U02`, `TM-B01`). | Hash, schema, sequence/gap, round-trip, partial-publication, Compose/image/package tamper, wrong-platform/architecture, downgrade, rollback, migration, restore, and recovery tests. |
...
flowchart LR
    ...
    Keyring --> Broker -->|"memory or tmpfs; never image/env/Compose"| Gateway
    Broker --> Worker
    Gateway <--> Volume
    Worker <--> Volume
...
| `TM-M01` | Deployment-mode integrity: Local Desktop is the safe default; Connect Remote and Host Remote require explicit, informed selection and separate configuration. No environment, listener, interface, Docker context, installer flag omission, or discovered server may infer a remote mode, widen a bind, enable synchronization, or reuse credentials across profiles. |
...
| `TM-V01` | Container secret and data custody: the OS keyring remains the local authority; a narrow broker delivers per-container, short-lived material through memory or tightly permissioned tmpfs without Compose, image, environment, arguments, logs, or inspectable metadata. SQLCipher keys, identity secrets, encrypted volumes, migrations, backups, restore, rotation, revocation, and uninstall are fail-closed, versioned, recoverable, and tested for loss as well as disclosure. |
| `TM-G01` | Remote edge and identity: only the TLS reverse proxy is public; TLS and certificate validation, exact external origin, trusted-proxy allowlists, OIDC issuer/audience/redirect/PKCE/state/nonce, session rotation/revocation, CSRF protection, rate limits, default-deny authorization, administrative re-authentication, and clock-skew bounds pass preflight before Host Remote starts. No anonymous health, docs, setup, bootstrap, or internal service route is exposed. |
...
| `STR-B-S` | Spoofing: a registry, image tag, runtime release, package repository, publisher, platform, or architecture is impersonated. | `TM-B01`, `TM-U01`, `TM-U02` | #132, #353, #358-#362 / G7 | Negative: mutable tag, wrong digest/registry/publisher/platform/architecture, mirror substitution, unknown/revoked key, and expired metadata fail before execution. |
| `STR-B-T` | Tampering: an OCI layer, Compose model, bootstrap binary, package script, repository metadata, or downgrade alters the deployment. | `TM-B01`, `TM-U02` | #132, #353, #358-#362 / G7 | Negative: hash/size/SBOM/provenance mismatch, unsafe package hook, post-validation mutation, rollback/freeze, interrupted acquisition, and cross-channel conflict preserve the prior trusted version. |
...
| `AB-19` | A registry, mutable tag, wrong-architecture image, bootstrap package, or package repository supplies a compromised deployment. | Medium / Critical | `TM-B01`, `TM-U01`, `TM-U02`; #132, #353, #358-#362; G7. Negative: digest/platform/provenance/SBOM/license mismatch, rollback/freeze, mirror substitution, unsafe hook, and interrupted acquisition preserve the trusted version. | Not reduced: publication and acquisition evidence pending. |
