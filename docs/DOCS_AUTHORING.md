...
the released bounded 0.6.0 Electron desktop control shell. Its supported
destinations are Home, Diagnostics, Settings, and capability onboarding; it
uses only the authenticated health/capability sidecar. The 0.6.0 source also
contains source-level gated file-grant, provider-configuration, and
presentation-only Tasks adapters plus a synchronous, bounded transient-chat
API with exact profile, model, policy, and consent checks. The source-level
gated Chat destination consumes the Main-owned stream through fixed bridge
methods, renders hostile model output through a closed Markdown component
allowlist, and delegates plain-text copy and confirmed external HTTPS links to
Main. Target-matched packaged chat evidence, genealogy/domain task admission or
execution, direct artifact access, cloud accounts, and updater flows remain
planned or incomplete. State the supported release boundary, each source-level
gated adapter, and the remaining scope near desktop material; never present a
later capability as a current tutorial.
...
| `ADR-0027-core-package-extraction-decision.md` | Supporting/control: architecture decision | `docs/ADR-0027-core-package-extraction-decision.md` | Retain | Maintainers | Accepted CORE-42 decision record | Core extraction policy | Understand standalone extraction decision rule outcomes | GEDCOM extraction decision; RootsMagic extraction decision | GEDCOM and RootsMagic extraction decision ADR | Records separate GEDCOM and RootsMagic extraction decisions, safety invariants, provisional evidence boundaries, and measurable revisit triggers. | Retain basename; no redirect needed |
...
| `how-to/desktop-gedcom-intake.md` | How-to guide | `docs/how-to/desktop-gedcom-intake.md` | Create in #115; retain basename | Desktop users and maintainers | Source-level GEDCOM intake; packaged release gates pending | GEDCOM intake | Inspect local sources and choose roots | desktop GEDCOM intake; validation findings; root selection | Inspect desktop GEDCOM sources | Inspects local GEDCOM sources, reviews bounded findings, and chooses roots in the source-level desktop workspace. | Retain unique basename; validated in flat Wiki namespace |
...
| `reference/DESKTOP_DIAGNOSTICS.md` | Reference | `docs/reference/DESKTOP_DIAGNOSTICS.md` | Create in #461; retain basename | Desktop users and maintainers | Implemented privacy-safe local diagnostic contract | Desktop diagnostics | Look up diagnostic records and support boundaries | desktop diagnostics contract; diagnostic event codes | Desktop diagnostics contract | Defines correlation, schema, stable codes, retention, fixed native actions, privacy, and support evidence rules. | Retain unique basename; validated in flat Wiki namespace |
| `reference/MUTATION_RECOVERY.md` | Reference | `docs/reference/MUTATION_RECOVERY.md` | Create in #200; retain basename | Maintainers and operators | Implemented native coordination; hosted acceptance pending | Mutation coordination | Look up ownership and restart recovery | mutation recovery; interrupted export; resource conflict | Durable mutation coordination and recovery | Defines private journal ownership, atomic publication, authorized recovery, and legacy separate-file compatibility. | Retain unique basename; validated in flat Wiki namespace |
| `explanation/DESKTOP_SHELL.md` | Explanation | `docs/explanation/DESKTOP_SHELL.md` | Moved in #261 with git mv | Maintainers and desktop control-surface users | Released bounded 0.6.0 shell; clearly marked source-level gated Tasks presentation and Chat presentation with audited transient-chat transport | Desktop architecture | Understand released shell limits | Electron desktop shell; desktop scope | Desktop shell | Explains the supported Home, Diagnostics, Settings, and capability-onboarding boundary plus the bounded source-level gated Tasks and Chat presentation, Main-owned audited transient-chat bridge, safe model-output rendering, and excluded domain authority. | Wiki basename retained; Pages route moved to the classified path and the prior route retired to avoid duplicate canonical sources |
| `reference/DESKTOP_SIDECAR.md` | Reference | `docs/reference/DESKTOP_SIDECAR.md` | Moved in #261 with git mv | Desktop maintainers | 0.6.0 control sidecar; source-level gated fixed job lifecycle, audited transient-chat adapters, and bounded Chat consumer; admission and domain routes excluded | Desktop architecture | Look up supported sidecar constraints | desktop sidecar; Electron sidecar | Packaged desktop sidecar | Defines the supported private health and capability routes plus source-level gated lifecycle and bounded chat-stream routes, bridge contracts, renderer ownership, and native-action constraints; excludes task admission, genealogy, arbitrary provider selection, cloud, and updater authority. | Wiki basename retained; Pages route moved to the classified path and the prior route retired to avoid duplicate canonical sources |
...
| `reference/api/API_REFERENCE.md` | Reference | `docs/reference/api/API_REFERENCE.md` | Moved in #261 with git mv | Adapter authors | Implemented bounded 0.6.0 control API plus source-level gated fixed chat-stream routes | FastAPI capability foundation | Look up health and capability API | API reference; health capability endpoint | API reference | Describes the authenticated supported control API and the bounded source-level gated private chat-stream routes used by Electron Main. | Wiki basename retained; Pages route moved to the classified path and the prior route retired to avoid duplicate canonical sources |
...
| `release-evidence/issue-170-core-extraction-benchmark-and-dependency-report.md` | Supporting/control: release evidence | `docs/release-evidence/issue-170-core-extraction-benchmark-and-dependency-report.md` | Retain | Maintainers | Implemented CORE-42 decision evidence | Core extraction evidence | Review measured benchmark and dependency inputs | CORE-42 benchmark report; extraction dependency analysis | CORE-42 extraction benchmark and dependency report | Records methods, seven-run medians, RSS, consumer/dependency evidence, and release-burden analysis used by ADR-0027. | Retain issue-scoped pathname; no redirect needed |
| `release-evidence/issue-453-test-suite-audit.md` | Supporting/control: release evidence | `docs/release-evidence/issue-453-test-suite-audit.md` | Retain | Release maintainers | Implemented v0.7.0 maintenance evidence | Test-suite maintenance | Review test-suite consolidation evidence | test-suite audit; test consolidation; coverage evidence | v0.7.0 test-suite consolidation audit | Records the issue 453 consolidation rationale and equivalent before-and-after test evidence. | Retain issue-scoped pathname; no redirect needed |
...
| `release-notes/0.6.0.md` | Supporting/control: release note | `docs/release-notes/0.6.0.md` | Retain | Users and release maintainers | Version 0.6.0 release record | Release notes | Review version 0.6.0 changes | AncestryLLM 0.6.0 release notes | AncestryLLM 0.6.0 release notes | Records scope, limitations, installation, and verification for version 0.6.0. | Retain versioned pathname |
