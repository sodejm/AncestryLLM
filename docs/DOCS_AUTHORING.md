...
| `ADR-0026-local-first-container-remote-deployment.md` | Supporting/control: architecture decision | `docs/ADR-0026-local-first-container-remote-deployment.md` | Retain | Maintainers and operators | Accepted target; implementation and release gates open | Deployment architecture | Understand local container and remote profiles | local container deployment; remote hosting; deployment trust boundary | Local-first container and advanced remote deployment ADR | Records the accepted future deployment profiles, trust boundaries, ownership, budgets, and release gates without claiming current support. | Retain basename; no redirect needed |
