## AncestryLLM documentation

- [Documentation site](https://sodejm.github.io/AncestryLLM/)
- [GitHub Wiki](https://github.com/sodejm/AncestryLLM/wiki)
- [Home](Home.md)

### How-to guides

- [Interactive console guide](CONSOLE.md)
- [Encrypted backup and recovery](ENCRYPTED_BACKUPS.md)
- [First-run storage diagnostics](SETUP_DIAGNOSTICS.md)
- [Privacy and consent](PRIVACY_AND_CONSENT.md)
- [Provider guide](PROVIDERS.md)
- [Release runbook](RELEASING.md)
- [Wiki synchronization](WIKI_SYNC.md)
- [Wiki operations and recovery](WIKI_OPERATIONS.md)
- [Deployment operations](DEPLOYMENT.md)
- [Security response checklist](SECURITY_RESPONSE.md)

### Reference

- [CLI reference](CLI.md)
- [Command executor contract](COMMAND_EXECUTOR.md)
- [Application-service contracts](APPLICATION_CONTRACTS.md)
- [Architecture ownership and dependency contracts](ARCHITECTURE_CONTRACTS.md)
- [Bounded file ingress](FILE_INGRESS.md)
- [Versioning and compatibility](VERSIONING.md)
- [GEDCOM compatibility and release checks](GEDCOM_COMPATIBILITY.md)
- [Built-in module authoring](MODULE_AUTHORING.md)

### Explanation

- [REPL architecture and compatibility boundary](REPL_ARCHITECTURE.md)
- [Data-flow threat model and control matrix](THREAT_MODEL.md)
- [Desktop shell](DESKTOP_SHELL.md)
- [Packaged desktop sidecar](DESKTOP_SIDECAR.md)
- [Desktop verification](DESKTOP_VERIFICATION.md)
- [Continuous integration](CI.md)
- [Local LLM benchmarks](LOCAL_LLM_BENCHMARKS.md)
- [Local-first retrieval evaluation](LOCAL_RETRIEVAL_EVALUATION.md)

### Infrastructure and authoring

- [Documentation authoring rules](DOC_AUTHORING.md)
- [Documentation inventory and migration matrix](DOC_INVENTORY.md)

### Architecture Decision Records

- [ADR-0024: provider framework evaluation](ADR-0024-provider-framework-evaluation.md)
- [ADR-0025: Electron and FastAPI desktop](ADR-0025-electron-fastapi-desktop.md)
