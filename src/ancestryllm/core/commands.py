"""Transport-neutral command specifications shared by every input adapter."""
...
class DispatchKey:
    """Stable identity for one command action, independent of its transport."""
    ...
    @property
    def value(self) -> str:
        """Return the canonical serialization used by registries and diagnostics."""
...
@dataclass(frozen=True, slots=True)
class CommandRoute:
    """One action specification paired with its stable dispatch identity."""
...
@dataclass(frozen=True, slots=True)
class CommandSpec:
    """A command shared by one-shot, interactive, and future transports."""
    ...
    @property
    def routes(self) -> tuple[CommandRoute, ...]:
        """Derive complete dispatch metadata from this single specification."""

        return tuple(
            CommandRoute(DispatchKey(self.name, action.name), action) for action in self.actions
        )

    def route(self, action_name: str) -> CommandRoute:
        """Resolve one action or raise ``KeyError`` without transport-specific errors."""

        for route in self.routes:
            if route.action.name == action_name:
                return route
...
class ModuleDescriptor:
    """Composition metadata for one explicitly supported built-in module."""
    ...
    def command(self) -> CommandSpec:
        """Return command metadata without importing an implementation or UI package."""

        return COMMAND_SPECIFICATIONS[self.module_id]

    @property
    def actions(self) -> tuple[str, ...]:
        """Retain the descriptor API while deriving names from command metadata."""
