...
The supported 0.5.0 product surface is Home, Diagnostics, a sanitized capability summary, and local visual Settings only. It has no genealogy, files, jobs, chat, providers, cloud accounts, or updater controls. See the [desktop shell guide](../docs/DESKTOP_SHELL.md) for supported targets, manual installation, unsigned-artifact limits, and recovery guidance.

The persisted schema contains only color scheme, reduced-motion choice, onboarding completion, schema version, and optimistic revision. `onboardingCompleted` is internal workflow state, not a Settings control. Writes are validated, serialized, and atomically replace the file. Missing or supported legacy data receives safe defaults; corrupt or unsupported data is not silently overwritten. Provider configuration, accounts, file grants, genealogy data, prompts, payloads, and secrets are never preference fields.
...
| `TM-U01` | Static package-policy tests in `scripts/package-security.test.mjs`; packaged `app.asar`, fuse, and supported integrity inspection in `scripts/inspect-package-fuses.mjs`. Manual signing, notarization, provenance, target execution, and installation remain release gates; updater behavior is excluded from 0.5.0. | `make desktop-security` |
