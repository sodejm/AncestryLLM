...
const shellSpec = await readFile(new URL('../e2e/shell.wdio.ts', import.meta.url), 'utf8')
...
for (const scriptName of ['test:accessibility', 'test:visual']) {
  test(`${scriptName} selects at least one checked-in Electron scenario`, () => {
    ...
    assert.ok(grep, `${scriptName} must select a bounded WebdriverIO scenario with --grep`)
