...
replaces a key as part of diagnostics. Outside the fixed Issue #110 chat
...
- POSIX launch uses an isolated process group and bounded `SIGTERM`/`SIGKILL`
  ...
  so shutdown clears every transient session and message. Future provider
...
- A loopback-only FastAPI sidecar authenticates every request before body
  ...
  endpoint identity, credentials, and consent for every run. They expose no
  provider selector, generic dispatch, tools, filesystem, SQL, genealogy, or
  streaming surface. Other domain and provider-execution routers remain
