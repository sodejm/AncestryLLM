...
The native desktop workbench adds a separate `RootsMagicPresetQueryRequest`
boundary for People, Family links, and Events. Schema-aware statements bind
parameters and paginate in SQLite; browsing does not materialize the export
adapter's complete tables. Existing SQL/question CLI and REPL requests retain
...
scoped output grant authorizes one new destination. Final source verification,
revocation ownership, and exclusive same-filesystem publication share a short
commit guard. A committed job result wins over concurrent cancellation; source
