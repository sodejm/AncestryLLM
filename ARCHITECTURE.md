...
fixed settings/credential methods, and six fixed provider-configuration and
consent methods. Settings expose only reviewed non-secret metadata and
...
only adapts list, status, cancel, SSE replay, and shutdown routes. It adds no job
producer or submission route and no renderer job bridge. Long-running
