...
restart reconciliation. This boundary adds no renderer chat presentation,
genealogy operation, generic provider dispatch, or public API; Issue #112 owns
renderer conversation and Markdown/XSS behavior.
...
replaces a key as part of diagnostics. Outside the fixed Issue #110 and #111 chat
...
| `desktop/` | UI-only Electron adapter governed by ADR-0025. Its bounded first-run and Home-based welcome review, Home, Diagnostics, sanitized capability-summary, and local visual Settings surface now use Issue #106's reusable responsive presentation shell and fixed accessibility-state contracts; Issue #107 adds explicit local-only onboarding, a typed four-component startup report, read-only degraded recovery, and bounded retry; Issue #108 adds separate provider, endpoint-test, consent, privacy, limits, storage, and secret presentation; Issue #109 adds a bounded Tasks presentation, five fixed task-lifecycle requests, one validated event listener, backend snapshot reload, and sender-owned main subscriptions; sandboxed renderer; six 0.5 control methods plus three scoped opaque file-grant, five fixed settings/credential, six fixed provider-configuration/consent, five fixed task-lifecycle request, three fixed local-runtime, and three fixed chat-stream methods in Unreleased source; main-owned native dialogs, path mediation, revocation, and output locks; hardened main-process shell; fixed local protocol/CSP; global session/window denials; private native-sidecar supervisor and authenticated fixed-route clients; bounded main-owned durable preferences; Issue #104's main-process-only safe-shutdown preflight and native wait/cancel/stay decision; Issue #363's Main-only container-control foundation; Issue #348's policy-bound macOS arm64 runtime manager; local fuse/ASAR inspection; and unsigned unpacked package assembly are implemented. Settings, provider, runtime, task, and chat DTOs contain only reviewed non-secret fields, presence-only credential status, redacted endpoint identity, sanitized lifecycle state, coded failures, and path-free artifact metadata. Issue #109 adds no job producer, submission route, direct artifact action, provider execution, or genealogy operation; future artifact access remains grant-mediated. Issue #110 supplies the Python sidecar's fixed synchronous chat routes and service. Issue #111 adds a fixed Main/preload source bridge for start, cancel, acknowledgement, and owner-scoped batched events; Main owns strict SSE validation, a 16-millisecond or 4-KiB batch budget, a 256-KiB unacknowledged-byte ceiling, a 15-second stalled-renderer cancellation deadline, and one same-run cursor reconnect. It adds no renderer chat state or presentation. The #106 shell and #107 onboarding add no bridge, transport, service, or domain authority, and the fictional review gallery is excluded from production. The host container authority validates an app-owned endpoint and exact hardened plans; the local-runtime bridge can manage only the verified app-owned tool substrate, and it exports no socket, path, arbitrary arguments, or general process capability to the renderer or containers. No AncestryLLM application container is started. Renderer-owned provider access, rendered chat presentation, genealogy integration, domain routes, remote enrollment/hosting, and updating are excluded. A supported 0.x release requires a target-matched manually installed official unsigned installer and all release assurance gates; macOS and Windows prompts must be addressed by verifying published checksums and release evidence, and unsigned CI artifacts are not supported distribution packages. |
...
- Issue #111 implements the audited source transport for transient chat.
  ...
  once. Renderer conversation state, Markdown rendering, and XSS defenses remain
  Issue #112; packaged adversarial pressure evidence remains Issue #131.
- Issue #56 implements the service-internal asynchronous provider-stream
  ...
  its schema validation. Issue #111 consumes the adapter through private API and
  Electron Main ownership; renderer presentation remains #112, and no public
  stream authority is added.
...
preload. Neither API nor IPC owns provider policy, chat history, or renderer
presentation.
...
| Electron/internal API runtime | ADR-0025 was accepted and #98 is closed. The `0.5.0` foundation implements authenticated health and capability discovery, strict shared contracts, fail-closed loopback configuration, deterministic OpenAPI, the bounded Home/Diagnostics/Settings shell, first-run onboarding, six fixed control methods, the private native sidecar, payload verification, bounded supervision, process-tree cleanup, and durable main-owned preferences. Unreleased #103-#109 add the opaque file-grant broker, restart-safe job lifecycle, write-only credential/settings routes, responsive shell, startup diagnostics, provider profiles/consent, and a bounded Tasks presentation through fixed validated bridges. Unreleased #110 adds fixed synchronous transient-chat capability/session/run routes and a pure application contract over exact-profile, fresh-consent provider execution; content remains bounded process memory, failures do not append history, audit metadata excludes payloads, and the model receives no tools. Unreleased #111 adds fixed stream-start/SSE/cancel routes and a Main/preload source bridge. Main owns sender/run authorization, strict monotonic validation, 16-millisecond or 4-KiB batching, exact acknowledgements, a 256-KiB unacknowledged ceiling, 15-second stall cancellation, and one same-run cursor reconnect without retrying provider output. Shutdown and restart reconciliation produce exactly one payload-free terminal audit result. It adds no renderer chat state or model-output presentation, genealogy integration, domain or generic command route, updater, update feed, or background update channel. | A support or release claim for the Unreleased additions requires target-matched packaged verification and Issue #131's adversarial evidence. The Issue #103 dedicated packaged verification fixture is excluded from production and checked across the platform matrix; #110/#111 supply the bounded source-level provider-call and chat-transport boundaries, while native endpoint-network instrumentation, stalled-provider/renderer evidence, and packaged adversarial evidence remain #131. Renderer chat state and safe model-output rendering remain #112. Worker isolation, full parser budgets, atomic publication, target-matched screen-reader evidence, and packaged accessibility evidence remain owned by #114, #118, and #131. macOS and Windows can display an unknown-publisher or Gatekeeper prompt, so users must verify published checksums and release evidence before installation. Unsigned CI artifacts are verification inputs only. The manifest binding is not publisher signing or whole-bundle protection; #132 owns signing/notarization, and hosted exact-head Windows evidence remains the native process-tree proof. |
