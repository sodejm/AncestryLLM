...
| Requirement | Notes |
| --- | --- |
| [Docker Desktop](https://www.docker.com/products/docker-desktop/) | Runs the Open WebUI container and exposes `host.docker.internal`. |
...
5. **Bootstrap the stack**
   ...
   container stack with `docker compose up -d`:
...
> `langchain-ollama` (local LLM) and `google-generativeai` (Gemini OCR). Install
> them when you enable those features.
