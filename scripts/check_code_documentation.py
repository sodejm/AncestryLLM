...
# Pattern for a block comment opening, matched against the full file text.
_BLOCK_COMMENT_START_RE = re.compile(r"^\s*/\*", re.MULTILINE)

# Pattern for an HTML comment opening.
_HTML_COMMENT_START_RE = re.compile(r"^\s*<!--", re.MULTILINE)
...
def _has_file_level_comment(path: Path) -> bool:
    ...
    For multi-line block comments (/* … */ and <!-- … -->) we check whether the
    file starts with a block-comment opener, which is sufficient for a presence
    check.  Human review confirms the comment is genuinely useful.
    """
    try:
...
def check_inventory(tracked: list[str], root: Path) -> list[str]:
    ...
    ``NON_COMMENT_FORMAT_MAP`` so their semantics are discoverable.
    """
    ...
    for rel in tracked:
        ...
        if classification == "non-comment-format":
            if rel not in NON_COMMENT_FORMAT_MAP:
