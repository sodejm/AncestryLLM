"""Offline incremental synchronization for :mod:`tools.gedcom_merge`.

This module deliberately receives the merge module as a runtime dependency.
That keeps ``python tools/gedcom_merge.py`` working from any directory without
importing a second copy of the large core module under a different name.
...
class SyncError(RuntimeError):
    ...
    def render(self) -> str:
        ...
        lines.extend(
            f"  {index}. {fix}" for index, fix in enumerate(self.fixes, 1)
        )
        if self.details:
...
def _relative_lines(lines: Sequence[str], core: ModuleType) -> list[str]:
    ...
    return [
        f"{level} {tag} {_normal_value(tag, value, core)}".rstrip()
...
def _merge_citations(
    ...
    for tag in left_values.keys() | right_values.keys():
        if left_values[tag] and right_values[tag] and (
            left_values[tag] != right_values[tag]
        ):
            return None
    result = list(left)
        ...
        if child_tag in singleton_tags:
            ...
            for index, existing in enumerate(blocks):
                ...
                if _normal_value(child_tag, left_first.value, core) != _normal_value(
                    ...
                    return None
                if child_tag == "DATA":
                    merged_child = _merge_compatible_structure(
                        existing, child, core
                    )
    ...
    for record in (left, right):
        for block in core._top_level_blocks(record.lines):
            ...
            if key not in accumulator:
                ...
                accumulator[key] = _merge_same_fact(
                    accumulator[key], block, core, stats
                )
...
def _build_update_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--openai-model", default=os.getenv("OPENAI_MODEL", "gpt-5-mini")
    )
    parser.add_argument(
        "--gemini-model",
...
def _match_people(
    ...
    *,
    ...
    for source in sources:
        people = [
            _person_from_record(record, core)
    ...
    for index, spec in enumerate(specs, 1):
        ...
        previous_bindings = manifest.get("person_bindings", {}).get(
            spec.source_id, {}
        )
        bindings: dict[str, str] = {}
        inverse_pointer_map = {
            global_pointer: original
            for original, global_pointer in source.pointer_map.items()
        }
        for incoming in people_by_source[index]:
            ...
            candidate_pointers.update(
                fingerprint_index.get(_identity_fingerprint(incoming), ())
            )
            ...
            if len(safe_candidates) == 1:
                ...
                collection = (
                    stats.unchanged_people
                    if previous == target
                    else stats.mapped_people
                )
                ...
                safe_scored = [
                    ...
                    if item[0] >= 95 and item[2].automatic_merge_safe
                ]
                if len(safe_scored) == 1:
                    ...
                    if not assessment.conflicts:
                        try:
                            ...
                            if bool(verdict.get("is_duplicate")) and float(
...
def _map_nonpeople(
    ...
    master_records = [
        record for record in sources[0].records
        if record.tag not in {"HEAD", "TRLR", "INDI", "SUBM"}
    ]
    incoming_records = [
        record for source in sources[1:] for record in source.records
        if record.tag not in {"HEAD", "TRLR", "INDI", "SUBM"}
    ]
    ...
    for tag in ordered_tags:
        for record in (item for item in all_records if item.tag == tag):
            ...
            key_function = (
                _family_semantic_key if tag == "FAM" else _record_semantic_key
            )
            ...
            if existing:
                ...
                if tag == "SOUR" and record.pointer != target:
                    stats.source_records_consolidated.append(
                        f"{record.pointer} -> {target}"
                    )
            else:
                target = (
                    record.pointer
                    if is_master
                    else _next_pointer(tag, used, counters)
                )
            ...
            synthetic = core.GedcomRecord(
                rewritten, record.source_file, record.sequence
            )
...
def _block_has_citation(block: Sequence[str], core: ModuleType) -> bool:
    ...
    return any(
        core.parse_gedcom_line(line).tag == "SOUR" for line in block[1:]
    )
...
def _reconcile_person_blocks(
    ...
    *,
    ...
    block_registry: dict[str, dict[str, Any]] = copy.deepcopy(
        manifest.get("blocks", {})
    )
    ...
    for target, origin_records in grouped.items():
        ...
        for spec, record in origin_records:
            ...
            for block in core._top_level_blocks(rewritten):
                ...
                entry = person_registry.setdefault(key, {
                    "tag": tag,
                    "kind": "person-block",
                    "protected": [],
                    "observations": [],
                ...
                if spec is None:
                    ...
                    stats.added_facts.append(
                        f"{target}:{tag}:{spec.source_id}:{key[:12]}"
                    )
...
def _seed_snapshot_history(
    manifest: dict[str, Any], specs: Sequence[SnapshotSpec]
) -> None:
    """Record immutable snapshot metadata and replace active source pointers."""
    for spec in specs:
        manifest["snapshots"].setdefault(spec.snapshot_id, {
...
def _perform_update(args: argparse.Namespace, core: ModuleType) -> int:
    ...
    if all(
        manifest.get("active_snapshots", {}).get(spec.source_id)
        == spec.snapshot_id
        for spec in specs
    ):
        print(
    ...
    pointer_map, nonpeople = _map_nonpeople(
        sources, pointer_map, manifest, core, stats
    )
    ...
    for record in core.iter_gedcom_records(path):
        ...
        for block in core._top_level_blocks(record.lines):
            result[record.pointer][_block_key(block, core)] = (
                core.parse_gedcom_line(block[0]).tag
            )
    return result
...
def _perform_rebase(args: argparse.Namespace, core: ModuleType) -> int:
    ...
    for pointer, hashes in additions.items():
        ...
        for block_hash in hashes:
            entry = registry.setdefault(block_hash, {
...
def main(argv: Sequence[str], core: ModuleType) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 - core/SDK errors vary
        error = SyncError(
            "SYNC_OUTPUT",
